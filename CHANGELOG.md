...
### Fixed (2026-05-15)
- **Compose port lists concatenated instead of replaced in multi-file production setup.** When applying a base compose file plus a production override with `docker compose -f base.yml -f override.yml`, the `ports` list at each service was being concatenated, not replaced. Result: each service ended up with two host bindings — the private one from the override plus the public one inherited from the base — which silently exposed the backend, frontend, and supporting services on their original public ports, bypassing the front-door reverse proxy and its rate-limiting. Fix declares the merge strategy explicitly per service with `ports: !override` (Compose v2.24+) so the override list replaces rather than concatenates. Also surfaces a latent network split where the running containers had been created under a single-file compose invocation and lived on a different bridge than the one declared by the override file, so the runtime services declared in the override were unreachable. Resolved by recreating the affected service with `--force-recreate --no-deps` against both compose files together, plus removing a stale orphan container that the base file would otherwise try to start in this environment. End-to-end inference verified against a real dermatology photo: server-side path returns the full pipeline (vision classifier + LLM narrative) instead of the offline fallback.

### Fixed (2026-05-10)
- **Offline inference broken on mobile after the previous SW caching fix.** The .onnx model was already runtime-cached, but `ort.InferenceSession.create()` fetches the onnxruntime-web wasm runtime (~25 MB) before the model and hit `NetworkError` on the wasm fetch when offline. Root cause: the wasm was neither precached (`globPatterns` lacked `wasm`) nor runtime-cached (only the `/models/*.onnx` URL was matched). Fix adds a second `CacheFirst` rule for `*.wasm` with a dedicated `cacheName` so `cleanupOutdatedCaches` cannot cross-evict the model cache, plus a `prefetchModel()` rewrite that calls `loadModel()` (the only path that triggers ort to fetch BOTH the .onnx and its wasm in a single warmup). `useModelPrefetch` now guards on `isModelLoaded()` (in-memory session) instead of `isModelCached()` (only the .onnx) so users with a stale cache from before the fix still warm the wasm. Validated in field with airplane-mode Android: app falls cleanly to offline ONNX path, returns demodicosis classification with 56% confidence on the same canine dermatology photo as the previous regression report.
- **Workbox transactional install vs the 20 MB ONNX model.** Moved the model from `globPatterns` precache to `runtimeCaching` `CacheFirst`. Workbox precache is transactional: a single failed asset on a flaky mobile connection silently breaks the whole SW install, surfacing as "Offline model could not be loaded" with no .onnx in Cache Storage. Runtime caching downloads on demand and stores under a dedicated `cacheName` that survives `cleanupOutdatedCaches` across SW updates. `analyze.ts` now distinguishes "never downloaded" (recoverable, "connect once to download") from "download exists but load failed" (corrupt file or runtime issue) so the user gets a useful error message instead of a generic one.
